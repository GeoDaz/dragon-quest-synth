...
const Lower = memo(function Lower({
	className,
	described = true,
}: {
	className?: string;
	...
	const title = translateUI('Lower monster');
	const icon = (
		<svg viewBox="0 0 640 640" role="img" aria-label={title} focusable="false">
	...
	if (!described) {
		return <span className={makeClassName('lower', className)}>{icon}</span>;
	}
	return (
		<Described text={title} className={makeClassName('lower', className)}>
			{icon}
		</Described>
	);
});
