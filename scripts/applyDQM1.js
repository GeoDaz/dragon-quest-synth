...
const manual = JSON.parse(
	fs.readFileSync('./src/json/ignored/manual-renames.json', 'utf8')
);
...
const supp = {
	...
	'Tree Slime': 'Buddy Slime',
	'King Mottle Slime': 'Mottle King Slime',
	'Liquid Metal King Slime': 'Liquid Metal Slime King',
	'Dark Mecha-Slime': 'Dark Robot Slime',
	Swordgon: 'Stabosaur',
	Rayburn: 'Wyvern',
	'Smile Lizard': 'Raptile',
	Coatol: 'Flapdragon',
	Xiphos: 'Xiphos The Deathbringer',
	Tigertaur: 'Trigertaur',
	Taurus: 'Taurix',
	Veera: 'Hellsguard Veera',
	Florajay: 'Kingfuschia',
	Digster: 'Palooka Prawn',
	'Lucky Bag': "Bag O' Tricks",
	Lexicon: "Archfiend's Grimoire",
	'Stormgate Citadel': 'Stormsgate Citadel',
	Saggitar: 'Sagittar',
	'Tyrannasaura Wrecks': 'Tyrannosaura Wrecks',
	"Will-O'-Whips": "Will-O'-The-Whips",
	'Dead Masker': 'DeathMasker',
	...
	Rottney: 'Scruffy',
	Cottney: 'Fluffy',
	'Baboon Beast': 'Hihyurude',
	'Evil Beast': 'Hihyudorado',
	Maizarcane: 'Malzar Beast',
	Maizarugius: 'Zumergias',
	...
	Mechanowyrn: "Mechan-O'-Wyrm",
	'Goresdy-Purrvis': 'Lieutenant Goresby-Purrvis',
	'Goreham-Hogg': 'Lieutenant Goreham-Hogg',
	'Marquis de Leon': 'Marquis De Léon',
	Ogrodemir: 'Orgodemir',
	Thronella: 'Thornella',
	Roseguardian: 'Roseguardin',
	Goreilla: 'Gorerilla',
	'Scissor Beetle': 'Scissor Beatle',
	'King Godwin': 'King Godwyn',
	WhipBird: 'Gangleclaw',
	FireWeed: 'Flame Flower',
	BeanMan: 'Beanie Meanie',
	MechaDragon: 'DrakMachine',
	Dameselfly: 'Damselfly',
	'Swarm Troopers': 'Swarmtroop',
...
const locsByName = {};
for (const f in game)
	for (const r in game[f])
		for (const m of game[f][r]) {
...
for (const f in game)
	for (const r in game[f])
		for (const m of game[f][r]) {
			...
			let via = cand ? 'supp' : '';
			if (!cand && manual[m.name]) {
				cand = toKey(manual[m.name]);
				via = 'manual';
			}
			if (!cand) {
				cand = toKey(m.name);
			...
			if (!cand) {
				dropped.push([m.name, '(no match)']);
				continue;
			}
			if (cand === m.name) continue;
			// collision guard: target already used by another monster in the SAME family/rank
			const here = f + '/' + r;
			if (locsByName[cand] && locsByName[cand].has(here)) {
				dropped.push([m.name, cand + ' [collision same family/rank]']);
				continue;
			}
...
for (const f in game)
	for (const r in game[f])
		for (const m of game[f][r]) {
			if (finalMap[m.name]) {
				m.name = finalMap[m.name];
				renamed++;
			}
			for (const syn of m.synthesis)
				for (let i = 0; i < syn.length; i++) {
					if (/Family|Rank/.test(syn[i])) continue;
					if (finalMap[syn[i]]) {
						syn[i] = finalMap[syn[i]];
						refs++;
					}
				}
		}
...
Object.keys(finalMap)
	...
	.forEach(k => {
		const v = finalMap[k + '\0via'];
		viaCounts[v] = (viaCounts[v] || 0) + 1;
	});
const stillMissing = [];
for (const f in game)
	for (const r in game[f])
		for (const m of game[f][r]) if (!imgSet.has(m.name)) stillMissing.push(m.name);

console.log(
	'renamed:',
	Object.keys(finalMap).filter(k => !k.includes('\0')).length,
	'by source:',
	JSON.stringify(viaCounts),
	'synthesis refs:',
	refs
);
console.log('\n=== sample renames ===');
Object.keys(finalMap)
	.filter(k => !k.includes('\0'))
	.slice(0, 40)
	.forEach(k => console.log(`  "${k}" -> "${finalMap[k]}" (${finalMap[k + '\0via']})`));
...
if (!DRY) {
	fs.writeFileSync(gamePath, JSON.stringify(game, null, 4));
	console.log('\nWROTE ' + gamePath);
} else console.log('\n(dry run)');
